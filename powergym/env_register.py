...
def get_info_and_folder(env_name):
    ...
    folder_path = os.path.join(
        os.path.dirname(os.path.abspath(inspect.getsourcefile(Env))), "..", "systems"
    )
...
def make_env(env_name, dss_act=False, worker_idx=None):
    ...
    if worker_idx is None:
        ...
        base_file = os.path.join(
            folder_path, base_info["system_name"], base_info["dss_file"]
        )
...
def remove_parallel_dss(env_name, num_workers):
    base_info, folder_path = get_info_and_folder(env_name)
    base_main = os.path.join(
        folder_path, base_info["system_name"], base_info["dss_file"]
    )
    base_loadshape = os.path.join(
        folder_path, base_info["system_name"], "loadshape.dss"
    )
