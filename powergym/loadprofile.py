...
class LoadProfile:
    def __init__(self, steps, dss_folder_path, dss_file, worker_idx=None):
        ...
        for f in os.listdir(self.loadshape_path):
            low = f.lower()
            if ("load" in low) and low.endswith(".csv"):
    ...
    def create_file_with_daily(self, fname):
        ...
        for line in fin:
            if not line.lower().startswith("new load.") or ("daily" in line):
                ...
                line = line.strip()
                if "!" in line:
                    line = line[: line.find("!")].strip()  # remove inline comment
                if "//" in line:
                    line = line[: line.find("//")].strip()  # remove inline comment
    ...
    def add_redirect_and_mode_at_main_daily_dss(self, main_daily_dss):
        ...
        for line in lines:
            ...
            if "!" in low:
                low = low[: low.find("!")].strip()  # remove inline comment
            if "//" in low:
                low = low[: low.find("//")].strip()  # remove inline comment
            ...
            if (not redirect_load) and low.startswith("redirect"):
                if low.endswith("loads") or low.endswith("load"):
                    load_file = list(filter(None, line.strip().split(" ")))[
                        1
    ...
    def gen_loadprofile(self, scale=1.0):
        ...
        fscale = (
            np.genfromtxt(scale_txt).reshape(1)[0]
        ...
        # sort and output
        df = df.sort_values(by=["episode", "load", "step"])[
            ["episode", "load", "step", "mul"]
        ].reset_index(drop=True)
        for episode in range(episodes):
            if not os.path.exists(
                os.path.join(self.loadshape_path, str(episode).zfill(3))
            ):
            ...
            for load in self.LOAD_NAMES:
                ...
                series.to_csv(
                    os.path.join(
                        self.loadshape_path, str(episode).zfill(3), load + ".csv"
                    ),
                    header=False,
                    index=False,
                )
    ...
    def choose_loadprofile(self, idx):
        assert os.path.exists(
            os.path.join(self.loadshape_path, str(idx).zfill(3))
        ), "idx does not exist"
        ...
        with open(os.path.join(self.dss_folder_path, self.loadshape_dss), "w") as fp:
            for load_name in self.LOAD_NAMES:
                fp.write(
                    ...
                    + "mult=(file=./"
                    + os.path.join("loadshape", str(idx).zfill(3), load_name + ".csv")
                    + ")\n"
                )
